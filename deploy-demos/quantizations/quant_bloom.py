...
def load_quant(model, checkpoint, wbits, groupsize):
    ...
    model = model.eval()
    layers = find_layers(model)
    for name in ['model.transformer.word_embeddings', 'model.transformer.word_embeddings_layernorm']:
        if name in layers:
            del layers[name]
...
if __name__ == '__main__':
    import argparse
    from quant_datautils import get_loaders

    parser = argparse.ArgumentParser()

    parser.add_argument(
        'model', type=str,
        help='BLOOM model to load; pass `facebook/opt-X`.'
    )
    parser.add_argument(
        'dataset', type=str, choices=['wikitext2', 'ptb', 'c4', 'xz-instruct'],
