...
class DeepSpeedPPOTrainer():
    ...
    def _generate_sequence(self, prompts, mask):
        ...
        out_seq = []
        print_rank_0(f"pmt:{prompts}", self.args.local_rank)
        print_rank_0(f"seq:{seq}", self.args.local_rank)
        pmt= self.tokenizer.batch_decode(
            prompts.cpu().numpy().tolist(),
            # skip_special_tokens=True,
            # clean_up_tokenization_spaces=True
        )
        seq_str= self.tokenizer.batch_decode(
            seq.cpu().numpy().tolist(),
            # skip_special_tokens=True,
            # clean_up_tokenization_spaces=True
        )
        for i in range(len(pmt)): 
            print_rank_0(f"pmt/seq->{[pmt[i], seq_str[i] ]}", self.args.local_rank)
    ...
    def generate_experience(self, prompts, mask):
        ...
        with torch.no_grad():
            ...
            # print("returned values-ppo:",values)
            values = values.detach()[:, :-1]

        logits = output.logits
        logits_ref = output_ref.logits

        print_rank_0(f"score/values-raws: {reward_score}/{values}")
