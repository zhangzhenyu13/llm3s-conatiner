...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    if "Dahoas/full-hh-rlhf" in dataset_name:
        ...
        return raw_datasets.LmqgQagjaquadDataset(output_path, seed, local_rank,
                                                 dataset_name)
    elif "your-org/data-rlhf" in dataset_name:
        return raw_datasets.OurRLHFDataset(output_path, seed, local_rank,
                                                 dataset_name)
    else:
        raise RuntimeError(
...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
    ...
    reject_dataset = []
    if train_phase == 1:
        for i, tmp_data in tqdm.tqdm(enumerate(current_dataset), desc=f"phase{train_phase}"):
            # tokenize the text
        ...
        for i, tmp_data in tqdm.tqdm(enumerate(current_dataset), desc=f"phase{train_phase}"):
            ...
            if chosen_sentence is not None and reject_sentence is not None:
                ...
                reject_token = tokenizer(reject_sentence,
                                         ...
                                         truncation=True,
                                         return_tensors="pt")
                # print(f"tuning-->reject/chosen({tokenizer}):\n{reject_token['input_ids']}\n{chosen_token['input_ids']}")
