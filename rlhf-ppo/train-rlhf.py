...
def main():
    ...
    elif args.padding_side == "right":
        tokenizer.padding_side = "right"
    else:
        raise ValueError(f"padding side unknow:{args.padding_side}")

    print_rank_0(f"tokenizer-info:{tokenizer}",args.global_rank)
    print_rank_0(f'''{tokenizer("这个怎么处理？",
        max_length=64,
        padding="max_length",
        truncation=True,
        return_tensors="pt")}''', args.global_rank
    )
