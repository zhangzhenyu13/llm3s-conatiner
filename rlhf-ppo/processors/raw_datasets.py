...
def load_dataset(dataset_id):
    import os
    data= DatasetDict()
    data_path = os.path.join(os.environ['HOME'], "SharedData/RLHF/dataset", dataset_id)
...
#English dataset
class TaskSourceOasst1PairWiseRlhfrewardDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "tasksource/oasst1_pairwise_rlhf_reward"
        self.dataset_name_clean = "tasksource/oasst1_pairwise_rlhf_reward"
...
#English dataset
class AndersonbcdefgDollyRewardModelingPairwiseDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "andersonbcdefg/dolly_reward_modeling_pairwise"
        self.dataset_name_clean = "andersonbcdefg/dolly_reward_modeling_pairwise"
    ...
    def get_chosen(self, sample):
        if sample['preferred'] == "A":
            return sample['response_a']
        else:
            return sample['response_b']
    ...
    def get_rejected(self, sample):
        if sample['preferred'] != "A":
            ...
            return sample['response_b']
#English dataset
class AndersonbcdefgRedTeamingRewardModelingPairwiseNoAsAnAIDataset(AndersonbcdefgDollyRewardModelingPairwiseDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "andersonbcdefg/red_teaming_reward_modeling_pairwise_no_as_an_ai"
        self.dataset_name_clean = "andersonbcdefg/red_teaming_reward_modeling_pairwise_no_as_an_ai"
...
class AnthropicHhRlhfDataset(PromptRawDataset):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        self.dataset_name_clean = "Anthropic/hh-rlhf"
...
class CohereMiraclzhqueries2212Dataset(PromptRawDataset):
    ...
    def get_rejected(self, sample):
        if len(sample['negative_passages'])==0:
            print("warning: empty neg data----", sample)
            return None
...
class MkqaChineseDataset(PromptRawDataset):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "mkqa" #"mkqa-Chinese"
