...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    tf.logging.info('------->output pooler:{}<-------'.format(output_layer))
    with tf.variable_scope("loss"):
        # probabilities = tf.nn.softmax(output_layer, axis=-1)
        # log_probs = tf.nn.log_softmax(output_layer, axis=-1)
        
        # per_example_loss = -tf.reduce_sum(probabilities * log_probs, axis=-1) * 0
        # loss = tf.reduce_mean(per_example_loss)
        loss = output_layer[0,66] * 0

        return loss, output_layer
        # return (loss, per_example_loss, output_layer, output_layer)
...
def model_fn_builder(bert_config, 
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
                predictions={"pooler_output":probabilities},
                scaffold_fn=scaffold_fn)
        return output_spec
