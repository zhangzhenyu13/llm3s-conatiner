...
from mteb_xz.tasks import TaskInstructLoader
...
def load_dataset(*args):
    if len(args) > 1:
        dataPath = os.path.join(*args)
    else:
        dataPath = args[0]
    dataPath = os.path.join(os.environ['HOME'], 'SharedData', "mteb_zh", dataPath)

    data= DatasetDict()
    for key in os.listdir(dataPath):
        print("loading:", [args, key ] )
        data[key] = load_from_disk(os.path.join(dataPath, key))
    return data
...
class MedQQPairs(AbsTaskPairClassification, TaskInstructLoader):
    @property
...
class TNews(AbsTaskClassification, TaskInstructLoader):
    @property
    def description(self):
        return {
            'name': 'TNews',
...
class TYQSentiment(AbsTaskClassification, TaskInstructLoader):
    @property
    def description(self):
        return {
            'name': 'TYQSentiment',
...
class IFlyTek(AbsTaskClassification, TaskInstructLoader):
    @property
    def description(self):
        return {
            'name': 'IFlyTek',
...
class JDIphone(AbsTaskClassification, TaskInstructLoader):
    @property
    def description(self):
        return {
            'name': 'JDIphone',
...
class StockComSentiment(AbsTaskClassification, TaskInstructLoader):
    @property
    def description(self):
        return {
...
class GubaEastmony(AbsTaskClassification, TaskInstructLoader):
    @property
    def description(self):
        return {
            'name': 'GubaEastmony',
...
class T2RReranking(AbsTaskReranking, TaskInstructLoader):
...
class T2RRetrieval(AbsTaskRetrieval, TaskInstructLoader):
    def __init__(self, num_max_passages: int , **kwargs):
        super().__init__(**kwargs)
    ...
    def description(self):
        return {
            'name': f'T2RankingRetrieval-{self.num_max_passages}',
            'reference': 'https://huggingface.co/datasets/THUIR/T2Ranking',
            'type': 'Retrieval',
            'category': 's2p',
            'eval_splits': ['dev'],
    ...
    def load_data(self, **kwargs):
        ...
        corpus, queries, qrels = self.inst_inject((corpus, queries, qrels))
