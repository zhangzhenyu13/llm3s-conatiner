...
from data_collators import DataCollatorForRewardModel
...
def train():
    ...
    reward_model_path = os.path.join(args.work_space, model_config['reward_model']) 
    sft_model_path = os.path.join(args.work_space, model_config['sft_model']) 
    ...
    assert (
        reward_model_path and sft_model_path
    ), "Please specify a --reward/sft_model_path"
    ...
    tokenizer = AutoTokenizer.from_pretrained(sft_model_path, fast_tokenizer=True)
    ...
    reward_model =  RewardModel.from_pretrained(
        ...
        pad_token_id= tokenizer.pad_token_id, num_padding_at_beginning=0)
    print(reward_model)
    ...
    if lora_hyperparams is not None:
        ...
        print(config)
        sft_model = get_peft_model(sft_model, config)
        print(sft_model)
